...
def build_output_path(input_filename: str) -> str:
    ...
    return os.path.join(BGE_EMBEDDINGS_DIR, f"{stem}_bge_embeddings.json")
...
def embed_chunk_file(input_path: str, output_path: str):
    ...
    print(f"Embedding {len(texts)} chunks with {BGE_MODEL_NAME} ...")
    embeddings = embed_texts_semantic(texts, BGE_MODEL_NAME)
    ...
    result = {
        ...
        "embedding_dimension": BGE_EMBEDDING_DIMENSION,
    ...
    print(f"Saved BGE embeddings to {output_path}")


def main():
    os.makedirs(BGE_EMBEDDINGS_DIR, exist_ok=True)
